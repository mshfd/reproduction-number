(function (global, factory) {
      ...
      function BibtexParser() {
        ...
        this.value_braces = function () {
          ...
          while (true) {
            ...
            if (this$1.input[this$1.pos] == '\\' && escaped == false) { escaped = true; }
            else { escaped = false; }
            this$1.pos++;
          }
        };
        ...
        this.value_comment = function () {
          ...
          while (!(this.tryMatch("}", false) && brcktCnt == 0)) {
            ...
            if (this$1.input[this$1.pos] == '{') { brcktCnt++; }
            if (this$1.input[this$1.pos] == '}') { brcktCnt--; }
  ...
  var meta = function (dom, data) {
    ...
    function meta(name, content, force) {
      if (content || force) { appendHead(("    <meta name=\"" + name + "\" content=\"" + (index$1(content)) + "\" >\n")); }
    }
              ...
              journal: [
...

                {
                  journal_article: [
                    ...
                    {
                      contributors:
                        data.authors.map(function (author, ind) {
                          return (
                            {
                              person_name: [
                                {
  ...
  function citation_xml(key, ent) {
    ...
    if ("title" in ent) { info.push({ article_title: ent.title }); }
    if ("author" in ent) { info.push({ author: ent.author.split(" and ")[0].split(",")[0].trim() }); }
    if ("journal" in ent) { info.push({ journal_title: ent.journal }); }
    if ("booktitle" in ent) { info.push({ volume_title: ent.booktitle }); }
    if ("volume" in ent) { info.push({ volume: ent.volume }); }
    if ("issue" in ent) { info.push({ issue: ent.issue }); }
    if ("doi" in ent) { info.push({ doi: ent.doi }); }
